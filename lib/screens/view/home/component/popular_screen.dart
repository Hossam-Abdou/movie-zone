    ...
    return BlocBuilder<MovieCubit, MovieState>(builder: (context, state) {
      ...
      return GridView.builder(
        ...
        itemBuilder: (context, current) => InkWell(
          onTap: () {
            cubit.GetReview(
                cubit.popularMovieModel!.results![current].id as int);
            cubit
                .getGenre(cubit.popularMovieModel!.results![current].id as int);
            cubit.GetCast(cubit.popularMovieModel!.results![current].id as int);
            cubit.getVideos(
                cubit.popularMovieModel!.results![current].id as int);
            pushNavigate(
              context,
              DetailsScreen(
                id: cubit.popularMovieModel?.results![current],
              ),
            );
          },
          child: Column(
            children: [
              SizedBox(
                ...
                height: 205.h,
                child: Image.network(
                  '${EndPoints.linkImage}/${cubit.popularMovieModel?.results?[current].posterPath}',
                ),
              ),
              SizedBox(
                height: 10.h,
              ...
              Text(
                '${cubit.popularMovieModel?.results?[current].title}',
                style: TextStyle(
